...
public class AbstractDoomAudio implements IDoomSound{
			...
			int		volume )
	{
	  ...
	  if (!sfx->data)
	  {
		...
		// Record origin coordinates so HeadlessSoundDriver can recalculate
		// per-player volume/separation for multiplayer sessions.
		if (ISND instanceof HeadlessSoundDriver) {
			if (origin != null) {
				((HeadlessSoundDriver) ISND).setPendingOrigin(origin.getX(), origin.getY(), true);
			} else {
				((HeadlessSoundDriver) ISND).setPendingOrigin(0, 0, false);
			}
		}
