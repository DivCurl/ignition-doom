...
public interface ISoundDriver {
    ...
    public static ISoundDriver chooseModule(DoomMain<?, ?> DM, CVarManager CVM) {
        ...
        // Phase 4.1: Check for headless mode (Ignition integration)
        // When running headless, use event capture driver instead of Java Sound API
        boolean headlessMode = "true".equals(System.getProperty("java.awt.headless"));

        if (headlessMode && !CVM.bool(CommandVariable.NOSOUND)) {
            // Headless mode with sound enabled - use event capture driver
            System.out.println("ISoundDriver: Headless mode detected, using HeadlessSoundDriver");
            driver = new HeadlessSoundDriver(DM, DM.numChannels);
        } else if (CVM.bool(CommandVariable.NOSFX) || CVM.bool(CommandVariable.NOSOUND)) {
            driver = new DummySFX();
        } else {
