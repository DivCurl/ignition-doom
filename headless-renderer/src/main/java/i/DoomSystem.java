...
public class DoomSystem implements IDoomSystem {
    ...
    public void Quit() {
        ...
        DM.commit();
        DM.CM.SaveDefaults();
        // Phase 5.2: In headless mode (Ignition Gateway), do NOT call System.exit() — it kills the entire JVM.
        // Instead, interrupt the game thread so SessionRunner can catch it and clean up gracefully.
        if ("true".equalsIgnoreCase(System.getProperty("java.awt.headless"))) {
            System.out.println("DOOM: Quit requested in headless mode — interrupting game thread");
            Thread.currentThread().interrupt();
            throw new RuntimeException("DOOM quit requested by user");
        } else {
            System.exit(0);
        }
    }
    ...
    public void Error(String error, Object... args) {
        ...
        // Phase 5.2: In headless mode, interrupt the game thread instead of calling System.exit(-1).
        if ("true".equalsIgnoreCase(System.getProperty("java.awt.headless"))) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("DOOM error: " + error);
        } else {
            System.exit(-1);
        }
    }
    ...
    // In headless mode (no display), skip the dialog and return true (accept).
    @Override
    public boolean GenerateAlert(String title, String cause) {
        if (java.awt.GraphicsEnvironment.isHeadless()) {
            System.out.println("GenerateAlert (headless): " + title + " — " + cause);
            return true;
        }
