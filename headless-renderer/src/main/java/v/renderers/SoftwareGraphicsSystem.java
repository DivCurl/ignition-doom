...
{   
    ...
    private V[] colormap(RendererFactory.WithWadLoader<T, V> rf) {
        ...
        // Nested ternaries with generic array casts confuse the Java 17 bytecode verifier
        // (it infers Serializable instead of Object[] at the areturn merge point).
        // Explicit if-else gives each return a clear Object[] on the stack. cf. VerifyError fix.
        if (bufferType == byte[].class) {
            // Indexed mode: COLORMAP lump can be used directly
            return colormapEnabled
                ? (V[]) rf.getWadLoader().LoadColormap()
                : (V[]) BuildLightsI(paletteTrueColor(playpal));
        } else if (bufferType == short[].class) {
            // HiColor: generate colormaps with lights
            return colormapEnabled
                ? (V[]) BuildLights15(paletteTrueColor(playpal), rf.getWadLoader().LoadColormap())
                : (V[]) BuildLights15(paletteTrueColor(playpal));
        } else {
            // TrueColor: generate colormaps with lights
            return colormapEnabled
